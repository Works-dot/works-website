...
// One-time removal of the 6 demo blog posts that the original seed created.
// The real articles are migrated from Squarespace; the samples must not stay
// on the live site. Flag-gated so editors' future posts are never touched.
async function deleteSeedSampleBlogPosts(strapi: any) {
  const store = strapi.store({ type: "plugin", name: "migrations" });
  const FLAG = "delete_seed_sample_blog_posts_v1";
  if (await store.get({ key: FLAG })) return;

  const sampleSlugs = [
    "ux-kutatasi-modszerek",
    "mobile-first-tervezes",
    "ai-a-ux-tervezesben",
    "akadalymentes-tervezes-jovoje",
    "mikro-interakciok-ux-titka",
    "design-system-epites",
  ];
  ...
  for (const slug of sampleSlugs) {
    try {
      const rows = await strapi.db
        ...
        .findMany({ where: { slug }, select: ["documentId"] });
      const documentIds = [...new Set(rows.map((r: any) => r.documentId))];
      for (const documentId of documentIds) {
        // documents().delete() removes both draft and published versions.
      ...
      return; // retry the whole cleanup on next restart; flag not set
    }
  }

  await store.set({ key: FLAG, value: true });
  strapi.log.info("Sample blog cleanup: completed");
}

// Migrates blog posts from scripts/squarespace-posts.json (generated by
// scripts/squarespace-extract.mjs from the Squarespace export) and PUBLISHES
// them. Idempotent: slugs that already have a published version are skipped
// (so editors' later changes are never overwritten); draft-only versions
// (from the earlier draft-only migration) are published as-is.
async function migrateSquarespacePosts(strapi: any) {
  const fs = require("fs");
  const path = require("path");
  ...
  for (const post of posts) {
    try {
      ...
      const existingRows = await strapi.db
        .query("api::blog-post.blog-post")
        .findMany({ where: { slug: post.slug }, select: ["documentId", "publishedAt"] });
      if (existingRows.some((r: any) => r.publishedAt)) {
        continue; // already published (or an editor created it) — never touch it
      }
      if (existingRows.length > 0) {
        // Draft-only leftover from the earlier draft-only migration — publish it.
      ...
      for (const block of post.blocks || []) {
        if (block.type === "text") {
          ...
          if (imgId) {
            // DB column is varchar(255) — truncate overly long captions.
            let caption = block.caption || "";
            if (caption.length > 255) caption = caption.slice(0, 252) + "...";
            contentBlocks.push({
